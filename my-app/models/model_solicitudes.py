# my-app/models/model_solicitudes.py
...
class SolicitudModel:
    ...
    def _parse_solicitante_data(self, datos):
        ...
        if tipo_solicitante == 'Comunidad':
            ...
            rif = datos.get('inst_telefono', '').strip() or f"I-{datos.get('inst_director_cedula', '').strip()}"
            parroquia = ''
            municipio = ''
            ...
            rif = f"P-{datos.get('part_cedula', '').strip()}"
            parroquia = ''
            municipio = ''
    ...
    def _parse_solicitud_data(self, datos):
        ...
        direccion = datos.get('part_direccion', '').strip() or datos.get('inst_direccion', '').strip() or datos.get('com_municipio', '').strip()
        return {
            'fecha': fecha,
            'telefono_solicitante': telefono,
            'direccion_solicitante': direccion,
            'tipo_solicitud': datos.get('tipo_solicitud', '').strip(),
            'estatus_solicitud': datos.get('estatus', '').strip(),
