...
# ==================== CARGA DE EVIDENCIAS (IMÁGENES) ====================

@informe_avance_bp.route('/api/evidencias/subir', methods=['POST'])
def subir_evidencias():
    """
    Sube evidencias fotográficas (antes, durante, después)
    Implementa compresión de imágenes (Prof. Cadenas)
    Limita a 5 imágenes por etapa
    """
    if 'conectado' not in session:
        return jsonify({'status': 'error', 'message': 'No autorizado'}), 403
    
    try:
        etapa = request.form.get('etapa')  # 'antes', 'durante', 'despues'
        ...
        # Validar límite de 5 imágenes (Prof. Cadenas)
        if len(archivos) > InformeAvanceModel.MAX_IMAGENES_POR_ETAPA:
            return jsonify({
                'status': 'error', 
                'message': f'Máximo {InformeAvanceModel.MAX_IMAGENES_POR_ETAPA} imágenes por etapa'
            }), 400
        ...
        for archivo in archivos:
            if archivo and allowed_file(archivo.filename):
                ...
                # Comprimir imagen (Prof. Cadenas)
                InformeAvanceModel.comprimir_imagen(filepath, calidad=85)
                
                # Guardar URL en lista
                urls_guardadas.append(filepath)
        
        return jsonify({
            'status': 'success',
            'message': f'{len(urls_guardadas)} imágenes subidas correctamente',
            'urls': urls_guardadas
        })
        
    except Exception as e:
        print(f"Error subir_evidencias: {e}")
        return jsonify({
            'status': 'error', 
